...
public class HunterAI : MonoBehaviour
{
    ...
    HunterMotion demon_m;
    HunterStatus demon_s;
    ...
    void Start()
    {
        ...
        demon_m = GetComponent<HunterMotion>();
        demon_s = GetComponent<HunterStatus>();
    ...
    IEnumerator AttakDelay()
    {
        while (!Exit)
        {
            ...
            if (!death && !player_die)
            {
                Shadow.GetComponent<HunterShadowMotion>().Attak(player_pos);
    ...
    void Update()
    {
        ...
        if ((dir >= 0.8f && dir < 4f || SunBbbng) && !player_die)
        {
            ray_dis = 0f;
        ...
        if (dir > 4f && !Idle && !death && !SunBbbng && !player_die)
        {
            ray_dis = 0.5f;
